...
class PersonServiceSpec extends FunSuite with Matchers with MockFactory  {
  ...
  test("Valid Find all of repository") {
    ...
    (personRepository.findAll _).expects().returning(Future.apply(Seq[Person](Person(1L, "1051", "Dario", new Date(new LocalDate(1990, 6, 8).toDate().getTime)))))
  ...
  test("Save Person Form") {
    ...
    val personService = new PersonServiceImpl(personRepository, messageBrokerService)
    val person = Person(0L,"1051", "Dario", new Date(new LocalDate(1990, 6, 8).toDate().getTime))
  ...
  test("Update Person Find By Id") {
    ...
    val personService = new PersonServiceImpl(personRepository, messageBrokerService)
    val person = Person(1L,"1051", "Dario", new Date(new LocalDate(1990, 6, 8).toDate().getTime))
